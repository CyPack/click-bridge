#!/usr/bin/env python3
"""click-bridge — a localhost bridge that writes browser click events to disk for a
terminal AI agent to read.

The browser side (a react-dev-inspector callback, a custom overlay, a userscript...)
POSTs to /click; the server atomically writes the latest click to <dir>/last.json and
appends it to <dir>/history.jsonl. The Claude Code hook injects last.json into the next
prompt's context.

Stdlib only. Binds to 127.0.0.1 only by default.
"""
import argparse
import json
import os
...
class ClickBridgeHandler(BaseHTTPRequestHandler):
    data_dir: Path  # set by make_server
    ...
    def log_message(self, fmt, *args):  # single-line stderr log
...
def _tailscale_ip():
    """Return the Tailscale IPv4 address (or None) — lets remote devices send clicks."""
    try:
...
def main():
    ...
    ap.add_argument("--bind", action="append",
                    help="extra bind address (repeatable); default: 127.0.0.1 + tailscale-auto")
