"""click-bridge server testleri — stdlib http.client ile gerçek server'a karşı."""
import json
import threading
import time
import http.client
...
@pytest.fixture()
def bridge(tmp_path):
    """Ephemeral portta gerçek server başlat, (host, port, dir) döndür."""
...
def test_get_last_returns_latest(bridge):
    host, port, _ = bridge
    _req(host, port, "POST", "/click", json.dumps({"selector": "#btn", "note": "kayik buton"}))
...
def test_invalid_json_400(bridge):
    host, port, _ = bridge
    status, _, data = _req(host, port, "POST", "/click", "{bozuk json")
    assert status == 400
    assert json.loads(data)["ok"] is False
