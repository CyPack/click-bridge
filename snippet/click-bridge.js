/**
 * click-bridge universal snippet — sends the Alt+Clicked element + diagnostic context to Claude Code.
 * Usage (dev-only): <script src="http://127.0.0.1:7823/snippet.js"></script>
 * Payload sent: component name (data-component > React fiber > tag), source file:line (data-file/line >
 * React _debugSource), CSS selector, text, box-model, LAST CONSOLE ERRORS, FAILED NETWORK REQUESTS, viewport.
 */
(() => {
  if (window.__clickBridgeLoaded) return;
  window.__clickBridgeLoaded = true;
  // Target the bridge on whatever host served the page (local: 127.0.0.1, remote: Tailscale IP/name)
  ...
  window.fetch = async (...a) => {
    ...
    if (url.includes(':7823')) return origFetch(...a); // don't watch the bridge itself
  ...
  // ── component chain + source from the React fiber (best-effort) ──────────────
  ...
  document.addEventListener('click', async e => {
    ...
    try {
      ...
      toast('click-bridge unreachable (' + _bh + ':7823) ' + err, false);
    }
  }, true);

  console.log('%c🖱️🎯 click-bridge active — Alt+Click to send to Claude Code', 'color:#7aa2f7;font-weight:bold');
})();
